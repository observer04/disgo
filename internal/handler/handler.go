...
// Config: for getting and setting configuration parameters; example: CONFIG GET maxmemory
func config(args []string, kv *store.Kv) (resp.Value, error) {
	if len(args) < 2 {
...
func keys(args []string, kv *store.Kv) (resp.Value, error) {
	if len(args) != 1 {
